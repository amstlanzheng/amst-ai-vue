...
const sendMessage = async (content?: string) => {
  ...
  try {
    ...
    if (currentRound.value >= MAX_ROUNDS) {
      isGameOver.value = true
      if (forgiveness.value >= 100) {
        gameResult.value = '恭喜你！在最后一轮成功哄好了猫猫！💕'
      } else {
        gameResult.value = `游戏结束：对话轮次已达上限(${MAX_ROUNDS}轮)，当前原谅值为${forgiveness.value}，很遗憾没能完全哄好猫猫`
      }
    }
    // 检查是否游戏结束
